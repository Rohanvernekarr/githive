  ...
  useEffect(() => {
    const fetchTrendingProjects = async () => {
      try {
        ...
        // Map the GitHub API response to our Project interface
        const formattedProjects: Project[] = data.items.map((repo: any) => ({
          id: repo.id,
          name: repo.name,
        ...
        {loading && (
          <div className="text-center py-10">
            <div className="inline-block h-8 w-8 animate-spin rounded-full border-4 border-solid
             border-white border-r-transparent"></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {projects.map((project) => (
              <div key={project.id} className="group">
                <div className="bg-zinc-900 rounded-lg shadow-md overflow-hidden transform transition
                 duration-300 ease-in-out 
                  ...
                  <div className="p-6 flex flex-col transition-all duration-300 h-full">
                    ...
                    <img 
                      ...
                      />
                      <h3 className="text-xl font-semibold ml-1 group-hover:text-purple-300 transition
                       duration-300">
                        {project.name}
                      </h3>
                      <span className="text-sm bg-zinc-700 group-hover:bg-zinc-600 px-3 py-1 
                      rounded-full transition duration-300 ml-3">
                        {project.language}
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between mt-auto">
                      ...
                      <span className="text-sm bg-green-800  px-3 py-1 
                      rounded-full transition duration-300">
                    ...
                    {/* Button that's visible on small screens by default and on hover for larger screens */}
                    <a 
                      href={project.url}
                      target="_blank" 
